    ...
    componentDidMount() {
        DictionaryService.cache.then(cache =>
            DictionaryService.global.then(global =>
                DictionaryService.local.then(local =>
                    this.setState({cache, global, local}),
                ),
            ),
        );
    }
    public render() {
        ...
        return (
            <div>
                <ConditionalRender
                    predicate={() =>
                        DictionaryService.globalUpdateAvailable.then(x => !x)
                    }
                >
                    <div style={style}>
                        <DictStats
                ...
                <ConditionalRender
                    predicate={() => DictionaryService.globalUpdateAvailable}
                    default={true}
                >
                    <p>
                        {' '}
                        Please sync your dictionary with the global dictionary before
                        proceeding{' '}
                    </p>
                </ConditionalRender>
            </div>
...
const DictStats: React.FC<DictStatsProps> = props => (
    <div>
