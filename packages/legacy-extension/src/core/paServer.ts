...
export class PAServer {
        ...
        return this.config.get(ConfigKeys.server_url).then<Res>(server_url => {
            return this.ajax({
                ...
                if (Messages.isNegativeResponse(parsedResponse)) {
                    console.error(
                        'Server returned failed response',
                        parsedResponse.errors,
                    );
                    return Promise.reject(
                        'Negative response: ' + JSON.stringify(parsedResponse.errors),
                    );
                }
    ...
    public openFolder(artist: Model.Artist) {
        console.debug(
            `openFolder called with artist { id: ${artist.id}, name: ${artist.name} }`,
        );
        return this.callEndpoint(Features.OpenToArtist, artist).catch(() =>
            this.openRepo(),
        );
    }

    public openImageFolder(image: Model.Image): Promise<void> {
        return this.callEndpoint<{image: Model.Image}, void>(Features.OpenToImage, {
            image,
        });
    }
    ...
    public download(artist: Model.Artist, imageUrl: string) {
        console.debug(
            `download called with artist { id: ${artist.id}, name: ${artist.name} } and imageUrl [${imageUrl}]`,
        );
        let msg: Messages.ArtistUrlRequest = {artist: artist, url: imageUrl};
        return this.callEndpoint(Features.DownloadImage, msg).then(() =>
            Promise.resolve(),
        );
    }
    ...
    public downloadAnimation(request: Messages.ArtistImageRequest, content: string) {
        console.debug(
            `download called with artist { id: ${request.artist.id}, name: ${request.artist.name} } and image [${request.image.id}]`,
        );
        let msg = Object.assign(request, {content});
        return this.callEndpoint(Features.DownloadAnimation, msg).then(() =>
            Promise.resolve(),
        );
    }
    ...
    public downloadMulti(artist: Model.Artist, imageUrls: string[]) {
        console.debug(
            `downloadMulti called with artist { id: ${artist.id}, name: ${artist.name} } and imageUrls of count [${imageUrls.length}]`,
        );
        let msg: Messages.BulkRequest<Messages.ArtistUrlRequest> = {
            items: imageUrls.map(url => ({artist, url})),
        };
        return this.callEndpoint(Features.DownloadManga, msg).then(() =>
            Promise.resolve(),
        );
    }

    public imageExistsInDatabase(
        artist: Model.Artist,
        image: Model.Image,
    ): Promise<boolean> {
        console.debug(
            `imageExistsInDatabase called with artist { id: ${artist.id}, name: ${artist.name} } and imageId [${image.id}]`,
        );
        let msg: Messages.ArtistImageRequest = {artist, image};
        return this.callEndpoint<Messages.ArtistImageRequest, boolean>(
            Features.ImageExists,
            msg,
        );
    }

    public bulkImageExists(
        entries: Messages.ArtistImageRequest[],
    ): Promise<Messages.ArtistImageRequest[]> {
        console.debug(`bulkImagesExist called with ${entries.length} entries`);
        return this.callEndpoint<
            Messages.BulkRequest<Messages.ArtistImageRequest>,
            Messages.ArtistImageRequest[]
        >(Features.ImagesExist, {items: entries}).then(result => {
    ...
    public supportedFeatures(): Promise<string[]> {
        return Promise.all<string>(
            Object.keys(Features).map(key =>
                this.supportsFeature((<any>Features)[key]).then(supported => {
                    if (supported) {
