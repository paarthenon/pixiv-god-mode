...
export function openFolder(artist: Model.Artist) {
	logger.debug(`openFolder called with artist { id: ${artist.id}, name: ${artist.name} }`);
	return callService(Features.OpenToArtist, artist);
}

export function download(artist:Model.Artist, imageUrl:string) {
	logger.debug(`download called with artist { id: ${artist.id}, name: ${artist.name} } and imageUrl [${imageUrl}]`);
...
export function downloadZip(artist: Model.Artist, zipUrl: string) {
	logger.debug(`download called with artist { id: ${artist.id}, name: ${artist.name} } and zipUrl [${zipUrl}]`);
...
export function downloadMulti(artist: Model.Artist, imageUrls: string[]) {
	logger.debug(`downloadMulti called with artist { id: ${artist.id}, name: ${artist.name} } and imageUrls of count [${imageUrls.length}]`);
...
export function imageExistsInDatabase(artist: Model.Artist, image: Model.Image, callback: (param: boolean) => any) {
	logger.debug(`imageExistsInDatabase called with artist { id: ${artist.id}, name: ${artist.name} } and imageId [${image.id}]`);
