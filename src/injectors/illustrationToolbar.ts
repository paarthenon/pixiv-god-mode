...
import {
    IllustrationToolbarContainer,
    IllustrationToolbarContainerProps,
} from 'src/components/illustrationToolbar';
import {GenerateElement} from 'src/injectors/utils';

import log from 'src/log';
import {awaitUntil} from 'src/utils/document';

export async function injectIllustrationToolbar(
    props: IllustrationToolbarContainerProps,
) {
    log.info('Injecting toolbar');
    let component = GenerateElement(
        React.createElement(IllustrationToolbarContainer, props),
    );
