...
let localImpl = defineImplementation<Msg.Protocol>('BACKGROUND_PAGE', {
    ...
    setConfig: msg => {
        return ChromeUtils.setConfig(msg.key, msg.value).then(ChromeUtils.handleError);
    },
    listConfig: () => {
        return ChromeUtils.listConfigKeys().then(ChromeUtils.handleError);
    },
    ajax: req => {
...
function firstTimeSetup() {
    ...
    let dictionaryService = new DictionaryManagementService(
        config,
    ...
    config
        .get(ConfigKeys.server_url)
        .catch(() => config.set(ConfigKeys.server_url, 'http://localhost:50415/'));
}
...
function setDebugMode(mode: boolean) {
    let config = new Config(localImpl);
    config
        .set(ConfigKeys.debug_mode, mode)
        .then(() => console.log('Debug mode set to', mode));
}
